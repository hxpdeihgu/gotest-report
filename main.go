...
func initRootCommand() (*cobra.Command, *templateData, *cmdFlags) {
	...
	rootCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (e error) {
			...
			allPackageNames, allTests,coverage, err := readTestDataFromStdIn(stdinScanner, flags, cmd)
			if err != nil {
			...
			tmplData.NumOfTestCoverage = coverage
...
func readTestDataFromStdIn(stdinScanner *bufio.Scanner, flags *cmdFlags, cmd *cobra.Command) (allPackageNames map[string]*types.Nil, allTests map[string]*testStatus,coverage string, e error) {
	...
	for stdinScanner.Scan() {
		...
		if goTestOutputRow.TestName != "" {
			...
			if strings.HasPrefix(goTestOutputRow.Output,"coverage") {
				coverage = strings.Replace(goTestOutputRow.Output,"coverage","Coverage",1)
			}
		}
	}
	return allPackageNames, allTests, coverage,nil
}
